...
server_socket.bind(('localhost', 8888))  # Changed to port 8888
    ...
    while True:
        try:
            ...
            for client_tuple in clients:
                if client_tuple[0] != client_socket:
                    try:
                        encrypted = encrypt_message(client_tuple[1], decrypted_message)
                        client_tuple[0].send(encrypted)
                    except:
                        # Mark disconnected client for removal
                        disconnected_clients.append(client_tuple)
